...
text_file1 = open("nikelink.txt", "w")
text_file1.write(r.text.encode('utf-8'))
text_file1.close()

reg = re.compile('<input type="hidden" name="action" value="(.*?)"/>.*<input type="hidden" name="lang_locale" value="(.*?)"/>.*<input type="hidden" name="country" value="(.*?)"/>', re.DOTALL)
